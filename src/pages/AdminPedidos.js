...
const AdminPedidos = () => {
  ...
  const fetchPedidos = async () => {
    try {
      ...
      const res = await axios.get('https://papeleria-backend.onrender.com/api/pedidos/admin', {
  ...
  const cambiarEstado = async (id_pedido) => {
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.put(
        `https://papeleria-backend.onrender.com/api/pedidos/admin/${id_pedido}/estado`,
        { estado: nuevoEstado },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const verProductos = async (id_pedido) => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`https://papeleria-backend.onrender.com/api/pedidos/admin/${id_pedido}/detalle`, {
        headers: { Authorization: `Bearer ${token}` },
      });
